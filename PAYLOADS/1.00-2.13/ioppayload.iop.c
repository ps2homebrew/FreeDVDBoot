...
void _start(void)
{
    ...
    // if (*(unsigned int *)READ_SECTORS_110 == addiu_magic)
    //     readSectors = (void *)READ_SECTORS_110;
    if (*(unsigned int *)READ_SECTORS_210 == addiu_magic)
        readSectors = (void *)READ_SECTORS_210;
    else if (*(unsigned int *)READ_SECTORS_212 == addiu_magic)
        readSectors = (void *)READ_SECTORS_212;
    else if (*(unsigned int *)READ_SECTORS_213 == addiu_magic)
        readSectors = (void *)READ_SECTORS_213;
    ...
    transfer_to_ee((void *)0x0FE5FF0, &return_address, sizeof(return_address)); // 2.12J

    transfer_to_ee((void *)0x1477B80, &return_address, sizeof(return_address)); // 2.13E/A
    ...
    for (i = 0; i < eh.phnum; i++) {
        ...
        while (remaining > 0) {
            ...
            if ((eph[i].offset + copied) & (SECTOR_SIZE - 1))
                k = SECTOR_SIZE - (eph[i].offset + copied) & (SECTOR_SIZE - 1);
